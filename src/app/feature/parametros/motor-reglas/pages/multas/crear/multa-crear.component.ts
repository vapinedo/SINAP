...
import { Concepto, CuotaMultaCreate, MultasCreate, MultasMunicipioAsignar, ParametroCreateMulta, ParametroCreateTarifa } from '@core/api/models';
...
@Component({
  selector: 'app-multa-crear',
  templateUrl: './multa-crear.component.html',
  styleUrls: ['./multa-crear.component.scss']
})
export class MultaCrearComponent implements OnInit, OnDestroy {
  ...
  multaCreate: MultasCreate;
  paramsListCreate : ParametroCreateMulta[] = [];
  cuotasListCreate: CuotaMultaCreate[] = [];
  municipiosListCreate: MultasMunicipioAsignar[] = [];


  public form = this.fb.group({
    nombreMulta:    ['', [Validators.required , Validators.pattern(new RegExp("\\S")), Validators.maxLength(200)]],
    normativa:    [''],
    ...
    cilindraje:   [''],
    cilindrajeMayor:   ['',[Validators.required, Validators.pattern(this.SOLO_NUMEROS_DIEZ)]],
    cilindrajeMenor:   ['',[Validators.required, Validators.pattern(this.SOLO_NUMEROS_DIEZ)]],
    cilindrajeFijo:   ['',[Validators.required, Validators.pattern(this.SOLO_NUMEROS_DIEZ)]],

    anioVehiculo:   [''],
    anioVehiculoMayor:   ['', [Validators.required, Validators.pattern(this.SOLO_NUMEROS_ANNIOS)]],
    anioVehiculoMenor:   ['', [Validators.required, Validators.pattern(this.SOLO_NUMEROS_ANNIOS)]],
    anioVehiculoFijo:   ['', [Validators.required, Validators.pattern(this.SOLO_NUMEROS_ANNIOS)]],

    anioInscripcion:   [''],
    anioInscripcionMayor:   ['', [Validators.required, Validators.pattern(this.SOLO_NUMEROS_ANNIOS)]],
    anioInscripcionMenor:   ['', [Validators.required, Validators.pattern(this.SOLO_NUMEROS_ANNIOS)]],
    anioInscripcionFijo:   ['', [Validators.required, Validators.pattern(this.SOLO_NUMEROS_ANNIOS)]],

    anioCirculacion:   [''],
    anioCirculacionMayor:   ['', [Validators.required, Validators.pattern(this.SOLO_NUMEROS_ANNIOS)]],
    anioCirculacionMenor:   ['', [Validators.required, Validators.pattern(this.SOLO_NUMEROS_ANNIOS)]],
    anioCirculacionFijo:   ['', [Validators.required, Validators.pattern(this.SOLO_NUMEROS_ANNIOS)]],

    aniosCirculacion:   [''],
    aniosCirculacionMayor:   ['', [Validators.required, Validators.pattern(this.SOLO_NUMEROS_ANNIOS)]],
    aniosCirculacionMenor:   ['', [Validators.required, Validators.pattern(this.SOLO_NUMEROS_ANNIOS)]],
    aniosCirculacionFijo:   ['', [Validators.required, Validators.pattern(this.SOLO_NUMEROS_ANNIOS)]],
  }, {validators: [
  ...
  getValoresSelect(){
    ...
    this.filtrosTablaSvc.tipoVehiculo().subscribe(console.log);
  ...
  cuotaArrayControls(i) {
    return (<FormArray>this.form.get('cuotasList')).controls[i];
    ...
    if(this.form.valid){
      if(!this.validarNombreMulta()){
        ...
        this.formControls.cuotasList.value.forEach(pago => {
          ...
          this.cuotasListCreate.push(cuota);
        });

          this.multaCreate = {
            nombre : this.formControls.nombreMulta.value,
            ...
            valorMinimoCuota: this.formControls.valorMinimoCuota.value,
            valorMaximoMulta: this.formControls.valorMaximoCuota.value,
            ...
            //Parametros
            parametrosList: this.paramsListCreate,

            //Cuotas
            numeroCuotas: this.formControls.numeroPagos.value,
            cuotasList: this.cuotasListCreate,
            
            municipiosList: this.municipiosListCreate

          }

            this.multaSvc.createMultaUsingPOST(this.multaCreate)
            .subscribe({
              next: data => {
          ...
          console.log('Multa Crear', this.multaCreate);
  ...
  validarNombreMulta(){
    this.formControls.nombreMulta.setValue(this.formControls.nombreMulta.value.trim());
    if(this.lstMultas.length != 0){
      return this.lstMultas.find(ele => ele.nombre == this.formControls.nombreMulta.value &&
                                      ele.estado == (this.formControls.estado.value == true ? 'A' : 'I') &&
                                      ele.detalleMultaList[ele.detalleMultaList?.length - 1]?.fechaFinVigencia >= this.formControls.fechaInicial.value.format('YYYY-MM-DD')  )
                                     
    }
    return false;
  }

  clearValidatorsParams(){
    this.formControls.cilindraje.disable();
    ...
    this.formControls.anioVehiculo.disable();
    ...
    this.formControls.anioInscripcion.disable();
    ...
    this.formControls.anioCirculacion.disable();
    ...
    this.formControls.aniosCirculacion.disable();
  ...
  crearListaParametros(){
    if(this.formControls.tipoVehiculo.value.id && this.formControls.tipoVehiculo.value.id != ''){
      ...
      this.paramsListCreate.push(parametro);
    }
    ...
    if(this.formControls.anioInscripcion.value && this.formControls.anioVehiculo.value !='NO_APL'){
      let parametro : ParametroCreateMulta = {
  ...
  openAsociarMunicipios(){
    const dialogRef = this.dialog.open(VentanaModalComponent, {
      ...
      data: { 
        dataComponent: {
          ...
          municipios: this.municipiosListCreate
        },
    ...
    .subscribe(municipios => {
      if (municipios) {
        this.municipiosListCreate = municipios;
      }
  })
  }
  ...
  changesCilindraje(){
    if(this.valoresFijos.includes(this.formControls.cilindraje.value)){
      ...
      this.formControls.cilindrajeMayor.disable();
      this.formControls.cilindrajeMenor.disable();
      this.formControls.cilindrajeMayor?.patchValue('', {emitEvent: false});
      this.formControls.cilindrajeMayor?.patchValue('', {emitEvent: false});
    }else if(this.valoresEntre.includes(this.formControls.cilindraje.value)){
      this.formControls.cilindrajeFijo.disable();
      this.formControls.cilindrajeFijo?.patchValue('', {emitEvent: false});
  ...
  changesAnioVehiculo(){
    if(this.valoresFijos.includes(this.formControls.anioVehiculo.value)){
      ...
      this.formControls.anioVehiculoMayor.disable();
      this.formControls.anioVehiculoMenor.disable();
      this.formControls.anioVehiculoMenor?.patchValue('', {emitEvent: false});
      this.formControls.anioVehiculoMayor?.patchValue('', {emitEvent: false});
    }else if(this.valoresEntre.includes(this.formControls.anioVehiculo.value)){
      this.formControls.anioVehiculoFijo.disable();
      this.formControls.anioVehiculoFijo?.patchValue('', {emitEvent: false});
  ...
  changesAnioInscripcion(){
    if(this.valoresFijos.includes(this.formControls.anioInscripcion.value)){
      ...
      this.formControls.anioInscripcionMayor.disable();
      this.formControls.anioInscripcionMenor.disable();
      this.formControls.anioInscripcionMayor?.patchValue('', {emitEvent: false});
      this.formControls.anioInscripcionMenor?.patchValue('', {emitEvent: false});
    }else if(this.valoresEntre.includes(this.formControls.anioInscripcion.value)){
      this.formControls.anioInscripcionFijo.disable();
      this.formControls.anioInscripcionFijo?.patchValue('', {emitEvent: false});
  ...
  changesAnioCirculacion(){
    if(this.valoresFijos.includes(this.formControls.anioCirculacion.value)){
      ...
      this.formControls.anioCirculacionMayor.disable();
      this.formControls.anioCirculacionMenor.disable();
      this.formControls.anioCirculacionMayor?.patchValue('', {emitEvent: false});
      this.formControls.anioCirculacionMenor?.patchValue('', {emitEvent: false});
    }else if(this.valoresEntre.includes(this.formControls.anioCirculacion.value)){
      this.formControls.anioCirculacionFijo.disable();
      this.formControls.anioCirculacionFijo?.patchValue('', {emitEvent: false});
  ...
  changesAniosCirculacion(){
    if(this.valoresFijos.includes(this.formControls.aniosCirculacion.value)){
      ...
      this.formControls.aniosCirculacionMayor.disable();
      this.formControls.aniosCirculacionMenor.disable();
      this.formControls.aniosCirculacionMayor?.patchValue('', {emitEvent: false});
      this.formControls.aniosCirculacionMenor?.patchValue('', {emitEvent: false});
    }else if(this.valoresEntre.includes(this.formControls.aniosCirculacion.value)){
      this.formControls.aniosCirculacionFijo.disable();
      this.formControls.aniosCirculacionFijo?.patchValue('', {emitEvent: false});
  ...
  consultarListaParametros(){
    let itemVehiculo = this.isVehiculoSelected ? this.formControls.tipoVehiculo.value.id : '';
